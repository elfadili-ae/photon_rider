import { companyLinks, otherLinks, productLinks } from '@/data'
...
const Footer = () => {
  return (
    <footer className="w-full max-container px-4 divide-y text-white">
		...
		<div className="grid grid-cols-2 text-sm gap-x-3 gap-y-8 lg:w-2/3 sm:grid-cols-4">
			<div className="space-y-3">
				...
				<ul className="space-y-1">
					{productLinks.map((ele, index) => {
						return <li key={index}>
						<Link rel="noopener noreferrer" href={ele.url}>{ele.name}</Link>
					</li>
						})
					}
				</ul>
			...
			<div className="space-y-3">
				<h3 className="uppercase dark:text-gray-900">More</h3>
				<ul className="space-y-1">
	...
	<div className="py-6 text-sm text-center dark:text-gray-600">© 2024 Photon Rider. All rights reserved.</div>
</footer>
  )
}
